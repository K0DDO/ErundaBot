...
Канал в `/config`. Пока канала нет — `/fest new` и `/fest ping` пишут туда, откуда вызвали. Одно сообщение = `#N`: сеанс (дата/время), заявки `@ник — Название` с постером справа (Кинопоиск/Википедия/iTunes, иначе без). После `/fest winner` остаётся только постер победителя, название крупно со случайным эмодзи сервера. Один фильм с человека. Все: `add`/`remove`. Роль «Кино»: `new`, `edit`, `delete`, `winner`, `export`, `ping`. `/fest delete` без номера — текущий открытый, с номером — любой старый (`#1`, `#2`…). `/fest ping` — если сеанс уже начался, «мы уже смотрим фильм». При старте бот обновляет карточки.

...
Несколько каналов на человека: ссылка `https://t.me/...`, `@channel` или invite `https://t.me/+...`. Название, картинка и описание с t.me; **раз в день в 04:00** (timezone гильдии) бот перезапрашивает t.me и обновляет доску. Каналы **сгруппированы по владельцу**, превью справа; ~7–8 на страницу, дальше «часть N из M». Ссылка — `[(приватка)]` / `[(открытый)]`, над ней серым описание (если есть). Кнопки «Добавить / Убрать ТГК» — на **последней** странице. `/tgk debug-add` — по нику (роль из `/config` или админ). БД: `tg_channels.description`, миграция v22.

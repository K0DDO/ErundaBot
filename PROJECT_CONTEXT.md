...
Канал в `/config`. Пока канала нет — `/fest new` и `/fest ping` пишут туда, откуда вызвали. Одно сообщение = `#N`: список фильмов без времени сеанса (`@ник — Название`), победитель внизу крупно с 🎬. Постер с Википедии/iTunes, если нет — без картинки. Один фильм с человека, повторный add заменяет. Все: `add`/`remove` и кнопка «Предложить фильм». Роль «Кино»: `new`, `edit`, `delete`, `winner`, `export`, `ping`. `/fest ping` — если сеанс уже начался, пишет «мы уже смотрим фильм». `/fest winner` ephemeral. Дебаг: `/fest role`, `/fest preview`, `/fest test-ping`. При старте бот обновляет уже существующие карточки.
